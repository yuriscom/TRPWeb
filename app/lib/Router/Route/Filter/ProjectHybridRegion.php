<?php
    ...
    class ProjectHybridRegion extends Filter {
        ...
        public function match($path, \Router\Route\Route $route, \Router\Router $router) {
            ...
            preg_match("/^(.*)-real-estate$/", $pathAr[0], $m);
            if (!isset($m[1])) {
                return false;
            }
            ...
            switch (count($pathAr)) {
                case 2:
                    break;
                case 3:
                    if ($pathAr[2] != 'new-preconstruction-for-sale') {
                        return false;
                    }
                    break;
                case 4:
        ...
        public function assemble(array $params) {
            ...
            $pattern = "/{province}-real-estate/{region}/new-preconstruction-for-sale(/{beds:[\"studio\",\"1\"]}-bedroom|/{beds}-bedrooms|/priced-between-{min_price}-{max_price}|/under-{max_price}|/over-{min_price}|/with-{pool}|/with-{concierge}|/with-{gym}|/move-in-{occupancy}|/by-{builder})";
            //            $pattern = "/{province}-real-estate/{region}/new-preconstruction-for-sale";
            $url = "";
            try {
