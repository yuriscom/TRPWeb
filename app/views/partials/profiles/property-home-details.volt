...
<div class="row">
	<div class="medium-6 columns small-padding-left">
		{% if listing['PropertyRooms']|length > 0 %}
			<div class="buttons">
			...
			{% for floor, rooms in listing['PropertyRooms'] %}
		...
		{% if full_images[1] is defined %}
			<div class="home-details-image">
				<div class="img-container">
					{{ partial('partials/images/profile-onpage', ['image': full_images[1], 'lazy': true, 'alt': address ~ ' is a ' ~ listing['PropertyTrpType']['name'] ~ ' for sale in ' ~ listing['addr_hood'] ~ ', ' ~ listing['addr_city']  ]) }}
				</div>
			</div>
		{% endif %}
	</div>
