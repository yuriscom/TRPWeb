require([
  ...
  , 'mortgage-tool-model'
    ...
    rShare, rCar, rContact, rFloor, s, MortgageToolView, MortgageToolModel) {
  ...
  var ProfileModel = Backbone.Model.extend({
    ...
    fetchScoreItems: function (type) {
      var self = this
      , propertyId = $('[data-profile-info]').data('profile-info').property_id
      , propertyTypeEndPoint = 'properties/';
  ...
  var ProfileView = Backbone.View.extend({
    ...
    initialize: function () {
      ...
      if ($('[data-inactive=true]').length === 0) {
        ...
        self.profileInfo = $('#exclusive-profile').data('profile-info');
        try {
          self.drawMap();
        } catch (error) {
          console.debug(error);
      ...
      var similarListingsLayer = self.profileInfo.precon_id ? 'projects' : 'properties';

      var similarListingsGallery = new GridListView({
        collection: Profile.similarPropertyCollection,
        el: $('#results-list'),
        layer: similarListingsLayer
      });
    ...
    initializeMortgage: function () {
      var profileInfo = $('#exclusive-profile').data('profile-info');
      var price;
      ...
      model.mortgage_listing_type = 'exclusive';
      model.mortgage_price = profileInfo.listing_price ?
        TheRedPin.helpers.parsePrice(profileInfo.listing_price) : 0;
      model.mortgage_tax_percent = profileInfo.listing_taxes ?
        TheRedPin.helpers.parsePrice(profileInfo.listing_taxes) / model.mortgage_price * 100 : 0;

      model.mortgage_rebate_amount = profileInfo.rebate_amount ?
        TheRedPin.helpers.parsePrice(profileInfo.rebate_amount) : 0;
      model.mortgage_maintenance = profileInfo.monthly_maintenance ?
        TheRedPin.helpers.parsePrice(profileInfo.monthly_maintenance) : 0;
